...
The CBM STS QA database will gather into one repository all relevant
 ...
 The data can then be directly accessed form any location either a web interface or ROOT macro.
 These information will be used to make decision on defect severity in order to accept 
 or reject a sensor under test for future use in STS detector.
...
process of importing, the measurement parameters are compared with the acceptance criteria
 and a corresponding flag being set. A screenshot is shown in Fig. \ref{ywu:fig:dbstrip}. 
...
\begin{figure}[htb]
	\centering
	\includegraphics[width=0.45\textwidth]{fig/ywu-dbstrip}
	\caption{Data query in command-line SQLite3 at CBMPC041}
