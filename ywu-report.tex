...
\abstract{This report introduces CBM-STS detector and the Quality Test Center(QTC)
	at GSI. It also details the hardwares for quality assurance as well as
	the QA procedures, which include bulk electrical, long-term stability and strip diagnostic tests. 
...
The STS will consist of eight planar tracking stations with about 900 readout modules 
 (see Fig.~\ref{ywu:fig:sts}). A module is comprised of a sensor, microcables
...
distribution to the testing centers, verification of specifications via tests and central
...
of the middle strip, and needle 2, 3, 4 contacts the AC pads of the neighboring strips, respectively.
...
\begin{figure}[htb]
	...
	\caption{Arrangement of the instruments for a multi-purpose strip diagnostic
		test. Four probe needles contact AC or DC pads of the neighboring strips~\cite{ywu:pl}.}
...
The CBM STS QA database will gather into one repository all relevant
 ...
 The data can then be directly accessed form any location using either a web interface or ROOT macro.
...
\begin{table}[t]
	...
	\begin{tabular}{|c|p{100pt}|}
		...
		\hline
		Pinhole, A & Current between implant and aluminum readout strip\\
		
		\hline
		Al\_Short\_L, A & Current between a given strip and its neighboring left strip\\
		
		\hline
		Al\_Short\_R, A & Current between a given strip and its neighboring right strip \\
...
\section{Summary}
In the two months study in GSI CBM-STS group, I have visited the STS laboratory and clean room at
